...
void main_menu(int cheatcode_unlocked) {
    ...
    int cheat_attempt = 0;
    while (fgets(choice, sizeof(choice), stdin)) { // fgets reads a line from stdin: fgets(variable, size of variable, source of input)
        choice[strcspn(choice, "\n")] = '\0'; // remove \n, cause fgets gets a whole line including \n
        if (!cheatcode_unlocked && (strcmp(choice, "1")==0) || strcmp(choice, "2")==0) { // go if cheatcode is locked. strcmp returns 0 if equal
            break;
        }
        else if (cheatcode_unlocked && (strcmp(choice, "1")==0 || strcmp(choice, "2")==0 || strcmp(choice, "3")==0)) { // go if cheatcode is unlocked
            break;
        }
        if (choice[0] == cheat_code[cheat_attempt]) { //as we have only one char to compare, we can use choice[0]
            cheat_attempt++;
            if (cheat_attempt == 11) {
                printf("Unclocked!\n");
                main_menu(1); 
